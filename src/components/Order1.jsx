...
const ClothDetailsForm = () => {
    ...
    const addShirtDetail = () => {
        setShirtDetails([...shirtDetails, { cloth: '', fitting: '' }]);
    };

    const addTrousersDetail = () => {
        setTrousersDetails([...trousersDetails, { cloth: '', fitting: '' }]);
    };
    ...
    return (
        <div style={formStyle}>
            <h1>Please enter the Cloth Details for each of the dress peice you need </h1>
            <div style={sectionStyle}>
                ...
                {shirtDetails.map((detail, index) => (
                    <div key={index} style={{ display: 'flex', alignItems: 'center' }}>
                        <input
                            type="text"
                            placeholder="Shirt Cloth Material online source (only cloth of registered brands are allowed)"
                            value={detail.cloth}
                            onChange={(e) => handleShirtChange(index, 'cloth', e.target.value)}
                            style={inputStyle}
                        />
                        <select
                            value={detail.fitting}
                            onChange={(e) => handleShirtChange(index, 'fitting', e.target.value)}
                            style={selectStyle}
                        >
            ...
            <div style={sectionStyle}>
                ...
                {trousersDetails.map((detail, index) => (
                    <div key={index} style={{ display: 'flex', alignItems: 'center' }}>
                        <input
                            type="text"
                            placeholder="Trousers Cloth Material online source (only cloth of registered brands are allowed)"
                            value={detail.cloth}
                            onChange={(e) => handleTrousersChange(index, 'cloth', e.target.value)}
                            style={inputStyle}
                        />
                        <select
                            value={detail.fitting}
                            onChange={(e) => handleTrousersChange(index, 'fitting', e.target.value)}
                            style={selectStyle}
                        >
                            <option value="">Select Fitting</option>
                            <option value="exact">Exact Fit</option>
